...
| A10 | [Ornithology Exchange (Ornithological Council)](https://ornithologyexchange.org/jobs/board/short-term-positions/) | Narrow-focus avian board — short-term positions, field techs, banding, RA roles. Point Blue lists this as a recruitment channel; the Idaho IBO IMBCR posting was here. | 5 |
| A11 | [Cyber-Sierra Natural Resources Jobs](http://www.cyber-sierra.com/nrjobs/) | Long-running aggregator focused on natural resources, ecology, wildlife, forestry. Surfaced via Yale SoE's curated list. | 4 |
| A12 | [NAAEE EE-PRO Jobs](http://naaee.org/eepro/jobs) | North American Assoc. for Environmental Education — interpreter, naturalist, visitor-education roles. Matches Felix's NPS Marin Headlands visitor talks experience. | 4 |
...
| 2026-05-25 | Added Ornithology Exchange (A10) after Point Blue's "how did you hear" referral list flagged it as a recruitment channel. Niche avian board, reliability 5. | Claude + Terry [source: cowork/laptop] |
| 2026-05-25 | Added Cyber-Sierra (A11) and NAAEE EE-PRO (A12) after Yale SoE's general-env-job-sites meta-list. Cyber-Sierra is NR/ecology-focused; NAAEE matches Felix's visitor-education work. Skipped EcoJobs (already A4), Climatebase (A6), and the Yale entries on our anti-list (Idealist, EnvironmentalCareer). | Claude + Terry [source: cowork/laptop] |
| 2026-06-01 | Weekly scrape — **0 new cards added.** Checked A (TAMU board, Conservation Job Board), C (Conservation Legacy/SCC, Corps Network), D (ACE-EPIC, CDFW/CalCareers), F (BayWork/EBMUD), I (Mote), and climate-tech (Quaise geothermal). Every candidate that could be fetched failed the hard URL-verification rule: TAMU board returned only April-2026 postings with passed deadlines; SCC Los Valles summer crew = "no longer accepting applications" (posted Dec 2025); BayWork/EBMUD Operator Trainee ad expired (Jan 2025 listing); ACE-EPIC Inyo Wilderness Permit Member = CLOSED, and the ACE-EPIC index renders JS-only (empty on fetch). ESLC Conservation Assistant + Mote internships could not be confirmed currently-open via WebSearch fallback. Quaise Energy was fresh (May 22) but its open roles are Facility Technician (building maintenance) and Field Technician (drilling rig) — facilities/mechanical, not env field science → dropped on role-type fit. No fabricated cards per safety rule. Existing 10 cards: none past deadline yet (earliest ERC 06-05, CRC 06-07 — both imminent). Sources needing maintenance: ACE-EPIC index + Conservation Job Board category pages are JS-rendered and not fetchable in this environment; web layer is returning stale snapshots board-wide. | Claude [source: cowork/scheduled-task] |
